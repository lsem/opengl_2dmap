...
bool load_file_content(std::filesystem::path fpath, std::string &out_data) {
  ...
  if (!fs) {
    log_err("failed opening file: {}; error: {} ({})", fpath, strerror(errno),
            errno);
    return false;
  }
...
inline std::unique_ptr<ShaderProgram>
make_from_fs_bundle(std::string fpath, std::string shader_idname) {
  using path_t = std::filesystem::path;
  auto base_path = path_t{fpath} / shader_idname;

  std::string v_shader_src;
  if (!load_file_content(path_t{fpath} / (shader_idname + ".vert.glsl"),
                         v_shader_src)) {
    log_err("failed loading file content of vertex shader for program: <{}>",
            shader_idname);
    return nullptr;
  }

  std::string f_shader_src;
  if (!load_file_content(path_t{fpath} / (shader_idname + ".frag.glsl"),
                         f_shader_src)) {
    log_err("failed loading file content of fragment shader for program: <{}>",
            shader_idname);
    return nullptr;
  }
