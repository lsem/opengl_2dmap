...
bool ImGui::BeginComboPreview() {
    ...
    if (window->SkipItems ||
        !window->ClipRect.Overlaps(
            g.LastItemData.Rect)) // FIXME: Because we don't have a ImGuiItemStatusFlags_Visible
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf, int buf_size,
    ...
    const bool focus_requested_by_code =
        (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_FocusedByCode) != 0;
...
bool ImGui::BeginMenu(const char *label, bool enabled) {
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        float icon_w = 0.0f; // FIXME: This not currently exposed for BeginMenu() however you can
                             // call window->DC.MenuColumns.DeclColumns(w, 0, 0, 0) yourself
...
bool ImGui::MenuItemEx(const char *label, const char *icon, const char *shortcut, bool selected,
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        pressed =
            Selectable("", false, flags | ImGuiSelectableFlags_SpanAvailWidth, ImVec2(min_w, 0.0f));
        RenderText(pos + ImVec2(offsets->OffsetLabel, 0.0f), label);
        if (icon_w > 0.0f)
