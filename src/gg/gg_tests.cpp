...
TEST(gg_tests, gpt_latlon_convertions) {
    ...
    for (double step = 0.1, prev_lat = -90.0, lat = prev_lat + step; lat <= 90.0; lat += step) {
        EXPECT_GT(gg::lat_to_y(lat), gg::lat_to_y(prev_lat))
            << "failed at: " << lat << "," << prev_lat;
        prev_lat = lat;
    }
    for (double step = 0.1, prev_lon = -180.0, lon = prev_lon + step; lon <= 180.0; lon += step) {
        EXPECT_GT(gg::lon_to_x(lon), gg::lon_to_x(prev_lon))
            << "failed at: " << lon << "," << prev_lon;
        prev_lon = lon;
    }

    EXPECT_NEAR(gg::y_to_lat(std::numeric_limits<uint32_t>::max() / 2), 90.0, 0.0001);
