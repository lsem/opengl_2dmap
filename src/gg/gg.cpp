...
EXTERN gpt_t wgs_to_gpt(double lat, double lon) { return {lon_to_x(lon), lat_to_y(lat)}; }
EXTERN wgs84_t to_wgs(const gpt_t &pt) { return {y_to_lat(pt.y), x_to_lon(pt.x)}; }
...
std::array<tile_id_t, 4> children_tiles(tile_at_level_t tile) {
    std::array<tile_id_t, 4> res = {tile_id_t{static_cast<uint16_t>(tile.id.x * 2 + 0),
...
std::vector<p32> eliminate_parallel_segments(std::vector<p32> points) {
    ...
    while (r < points.size() - 1) {
        ...
        if (std::abs(dot(a, br)) < 1e-5) {
            points[r].x = gg::U32_MAX; // todo: this is not good since we steal valid point
    ...
    points.erase(
        std::remove_if(std::begin(points), std::end(points),
                       [](const gg::p32 &v) { return v.x == gg::U32_MAX && v.y == gg::U32_MAX; }),
        std::end(points));
