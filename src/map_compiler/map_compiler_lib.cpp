...
shapes_t load_shapes(const fs::path &shape_file_path) {
  ...
  for (int i = 0; i < nEntities; i++) {
    std::unique_ptr<SHPObject, decltype(&SHPDestroyObject)> shape(
        SHPReadObject(lands_shp, i), &SHPDestroyObject);
    ...
    for (auto part_i = 0; part_i < shape->nParts; ++part_i) {
      ...
      if (part_points.size() < 3) {
        log_warn("discarding part {} of shape {}: less than 3 points", i,
                 part_i);
      } else {
